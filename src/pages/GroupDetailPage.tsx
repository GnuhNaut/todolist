...
const GroupDetailPage = () => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        ...
        <span className="ml-4 text-xl font-semibold text-gray-700">Loading group...</span>
      </div>
    );
  }
  ...
  const getDayName = (index: number) => ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][index];

  const formatRecurrence = (template: TaskTemplate) => {
    const { recurrence } = template;
    if (recurrence.type === 'daily') return 'Daily';
    if (recurrence.type === 'once') return `Once (${recurrence.startDate})`;
    ...
    <>
      <div className="max-w-5xl mx-auto p-4 md:p-8">
        <Link to="/" className="text-blue-600 hover:underline mb-4 inline-block group items-center">
          <span className="transition-transform group-hover:-translate-x-1 inline-block">&larr;</span> Back to Dashboard
        </Link>
          ...
          <div className="p-4 bg-white rounded-lg shadow-sm border border-gray-200">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
              <div className="mb-4 sm:mb-0">
                <h2 className="text-2xl font-semibold text-gray-800">Setup Task Templates</h2>
                <p className="text-gray-600 mt-1">
                  This is where you configure recurring tasks.
                </p>
              </div>
              <button
            ...
            <div className="mt-6">
              {taskTemplates.length === 0 ? (
                <p className="text-center text-gray-500 p-6 bg-gray-50 rounded-lg">No templates available.</p>
              ) : (
